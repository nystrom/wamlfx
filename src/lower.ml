...
let as_local_loc = function LocalLoc idx -> idx | _ -> failwith "unreachable"
let as_global_loc = function GlobalLoc idx -> idx | _ -> failwith "unreachable"
...
type clos_layout =
  { arity_idx : int32;
    code_idx : int32;
    env_idx : int32;  (* first environment entry *)
  }
...
let rec lower_val_type ctxt at rep t : W.val_type =
  ...
  | _, DropRep -> failwith "unreachable"
...
and lower_var_type ctxt at t : int32 =
  ...
  | _ -> Printf.printf "lower_var_type %s\n%!" (T.string_of_typ t); failwith "unreachable"
